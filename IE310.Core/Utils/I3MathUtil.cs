...
{
    public static class I3MathUtil
    {
        /// <summary>
        /// 10进制转换为2进制
        /// </summary>
        ...
        {
            AnyRadixConvert anyConvert = new AnyRadixConvert();
        ...
        public static string Convert10To16(long value)
        {
            ...
            int ys = 0;
            System.Math.DivRem(result.Length, 4, out ys);
            ys = 4 - ys;
        ...
        {
            return System.Math.Round(d, i, MidpointRounding.ToEven);
        }
        ...
        {
            return System.Math.Round(d, i, MidpointRounding.ToEven).ToString("F" + i.ToString());
        }
        /// <summary>
        /// 将金钱转换为大写人民币
        /// </summary>
        ...
        public static  int GetDecimals(double value)
        {
            ...
            if (index > 0 && index != valueStr.Length - 1)
            {
                valueStr = valueStr.Substring(index + 1);
                if (valueStr.Length > decimals)
                {
                    decimals = valueStr.Length;
                }
            }
            return decimals;
        }
    ...
    public class AnyRadixConvert
    {
            ...
            for (int i = 0; i < value.Length; i++)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        result += (long)System.Math.Pow(fromBase, i) * getcharindex(rDigits, value[value.Length - i - 1]);//   2
                    }
                    catch
                    {
        ...
        private string h2x(long value, int toBase)
        {
            ...
            int digitIndex = 0;
            long longPositive = System.Math.Abs(value);
            int radix = toBase;
        ...
        public string X2X(string value, int fromBase, int toBase)
        {
            ...
            {
                throw new ArgumentException(System.String.Format("The fromBase radix \"{0}\" is not in the range 2..36.", fromBase));
            }

            if (toBase < 2 || toBase > 36)
            {
                throw new ArgumentException(System.String.Format("The toBase radix \"{0}\" is not in the range 2..36.", toBase));
            }
