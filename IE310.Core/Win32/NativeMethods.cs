...
{
	/// <summary>
    /// 一个用P/Invoke方式封装Win32 API的类
	/// A class that contains methods for P/Invoking the Win32 API
	/// </summary>
	public abstract class I3NativeMethods
	{
		/// <summary>
        ...
        public static extern int DllGetVersion(ref I3DLLVERSIONINFO pdvi);
		...
		[DllImport("User32.dll", CharSet=CharSet.Auto, SetLastError=true)]
        public static extern bool TrackMouseEvent(I3TRACKMOUSEEVENT tme);
		...
		/// <summary>
        /// 移动窗体的绘画区 XAmount:X方向位移  YAmount:Y方向位移  lpRect:要移动的客户端区域  lpClipRect:剪切区域（有效区域）
		...
		[DllImport("User32.dll", CharSet=CharSet.Auto, SetLastError=true)]
        public extern static void keybd_event(byte bVk, byte bScan, I3KeyEventFFlags dwFlags, int dwExtraInfo);
